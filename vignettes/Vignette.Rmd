...
```{r}
order <- apply(assays(SEgenes)$pValue, 2, order, decreasing = F)
...
```{r}
order <- apply(assays(SEbins)$pValue, 2, order, decreasing = F)
...
```{r}
order <- apply(metadata(SEbins)$pValuesLocus, 2, order, decreasing = F)
